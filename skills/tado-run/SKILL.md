---
name: tado-run
description: ワークフローエンジン（tado）を起動し、init → next/report サイクルでワークフローを進行させる汎用ランナー。各ワークフロー Skill の薄い SKILL.md から --workflow <workflow.ts パス> 付きで起動される。ワークフロー固有のロジックは一切持たない。
---
...
```bash
tado init --workflow <workflow.ts パス>
```
